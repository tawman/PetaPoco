...
namespace PetaPoco
{
	...
	public class Database : IDisposable
	{
		...
		static Regex rxOrderBy = new Regex(@"\bORDER\s+BY\s+(?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|[\w\(\)\.])+(?:\s+(?:ASC|DESC))?(?:\s*,\s*(?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|[\w\(\)\.])+(?:\s+(?:ASC|DESC))?)*", RegexOptions.RightToLeft | RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
		...
		public static bool SplitSqlForPaging(string sql, out string sqlCount, out string sqlSelectRemoved, out string sqlOrderBy)
		{
			...
			// Look for the last "ORDER BY <whatever>" clause skipping any ROW_NUMBER() ORDER BY that exist prior
		...
		public void BuildPageQueries<T>(long skip, long take, string sql, ref object[] args, out string sqlCount, out string sqlPage) 
		{
			...
			{
				sqlSelectRemoved = rxOrderBy.Replace(sqlSelectRemoved, "", 1);
				if (rxDistinct.IsMatch(sqlSelectRemoved))
				{
